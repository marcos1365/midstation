...
function limpar(){
    ...
    contexto.fillStyle='black'
    //contexto.fillText(formas[numero],190,250)
    inserirImagem()
    gradiente()
    limparTouch()
}
...
function limparTouch(){
    ...
    ctx.fillStyle='black'
    //ctx.fillText(formas[numero],150,250)
    inserirImagemTouch()
}
...
function handleEnd(evt) {
    ...
    for (var i=0; i < touches.length; i++) {
        ...
        ctx.moveTo(ongoingTouches[idx].pageX, ongoingTouches[idx].pageY);
        ctx.arc(touches[i].pageX-el.getBoundingClientRect().x, touches[i].pageY-el.getBoundingClientRect().y-window.scrollY,10,0,Math.PI*2) 
        //window.scrollY resolve o problema da rolagem no celular
        gradient.addColorStop("0"," magenta");
        gradient.addColorStop("0.5", "blue");
        gradient.addColorStop("1.0", "red");
        ctx.fillStyle=gradient;
...
function inserirImagem(){
    var formas=['quadradoLinha','retanguloLinha','trianguloLinha','circuloLinha','losangoLinha',
                'pentagonoLinha','paralelogramoLinha','elipseLinha','trapezioLinha','estrelaLinha']
    var x=90, y=50, largura=200, altura=200
    if(numero==1||numero==6||numero==7||numero==8){x=70, y=70;largura=240; altura=160}
    const img = new Image()
    img.src = "imagens/"+formas[numero]+".png"
    img.onload = () => { contexto.drawImage(img, x, y, largura, altura)} 
}
function inserirImagemTouch(){
    var el = document.getElementsByTagName("canvas")[0];
    var ctx = el.getContext("2d"); 
    var formas=['quadradoLinha','retanguloLinha','trianguloLinha','circuloLinha','losangoLinha',
                'pentagonoLinha','paralelogramoLinha','elipseLinha','trapezioLinha','estrelaLinha']
    var x=50, y=50, largura=200, altura=200
    if(numero==1||numero==6||numero==7||numero==8){x=30, y=70;largura=240; altura=160}
    const img = new Image()
    img.src = "imagens/"+formas[numero]+".png"
